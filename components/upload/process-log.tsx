  ...
  return (
    <div className="mt-3">
      ...
      {isExpanded && (
        <div
          ref={scrollRef}
          className="mt-2 max-h-48 overflow-y-auto rounded-xl bg-gray-100 dark:bg-slate-950 border border-gray-200 dark:border-slate-800 p-3 text-xs leading-relaxed space-y-1"
        >
          {entries.map((entry, i) => (
            <div key={i} className="flex items-start gap-2 text-gray-600 dark:text-gray-300">
              <span className="text-gray-400 dark:text-gray-500 shrink-0 tabular-nums">{entry.time}</span>
